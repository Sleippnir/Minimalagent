...
class TranscriptData:
    ...
    def __init__(self, full_text_transcript: str, structured_transcript: List[TranscriptEntry]):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "structured_transcript": [entry.to_dict() for entry in self.structured_transcript]
        }
...
class Interview:
    ...
    def __init__(
        ...
        self.evaluation_materials = evaluation_materials or self._default_evaluation_materials()
        self.transcript_data = transcript_data or self._default_transcript_data()
        self.questions_and_answers = questions_and_answers or self._default_questions_and_answers()

        # Evaluation results - populated by LLM evaluation
        self.evaluation_1: Optional[Dict[str, Any]] = None  # OpenAI evaluation result
        self.evaluation_2: Optional[Dict[str, Any]] = None  # Google Gemini evaluation result
    ...
    def _default_transcript_data(self) -> TranscriptData:
        ...
        structured_entries = [
            TranscriptEntry("interviewer", "Hi, thanks for joining. Can you start by telling me about a challenging project you've worked on?"),
            TranscriptEntry("candidate", "Sure. In my last role, I was tasked with refactoring a monolithic legacy service into a microservices architecture. The main challenge was ensuring zero downtime during the migration. We used a strangler fig pattern to gradually move traffic to the new services. It was complex but ultimately successful."),
            TranscriptEntry("interviewer", "That sounds interesting. How would you handle a disagreement with a colleague about a technical implementation?"),
            TranscriptEntry("candidate", "I believe in open communication. I'd first listen to their perspective to fully understand their reasoning. Then, I would present my viewpoint with supporting data or examples. The goal is to find the best solution for the project, not to 'win' an argument. If we still can't agree, we could involve a third party, like a tech lead, for a final decision.")
        ]
        
        full_text = "\n\n".join([f"**{entry.speaker.title()}:** '{entry.text}'" for entry in structured_entries])
        
        return TranscriptData(
            full_text_transcript=full_text,
            structured_transcript=structured_entries
        )
    ...
    def to_llm_evaluation_format(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "questions_and_answers": [qa.to_dict() for qa in self.questions_and_answers]
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Interview':
        ...
        evaluation_materials = EvaluationMaterials.from_dict(data.get("evaluation_materials", {}))
    ...
    def from_json(cls, json_data: Dict[str, Any]) -> 'Interview':
        ...
        evaluation_materials = EvaluationMaterials(
            evaluator_prompt=json_data.get("interviewer_prompt", "Evaluate this candidate based on the interview."),
            rubric=rubric
        )
        ...
        if questions_data:  # Only process if questions_data is not None/empty
            for i, question in enumerate(questions_data, 1):
                if isinstance(question, str):
                    ...
                    qa = QuestionAnswer(i, question, "Evaluate based on candidate response")
                elif isinstance(question, dict):
                    # Structured question object
                    question_text = question.get("question", question.get("text", f"Question {i}"))
                    ideal_answer = question.get("ideal_answer", question.get("expected", "Evaluate based on candidate response"))
                    qa = QuestionAnswer(i, question_text, ideal_answer)
                else:
                    # Fallback
                    qa = QuestionAnswer(i, f"Question {i}", "Evaluate based on candidate response")
                
                questions_and_answers.append(qa)
        else:
            # No questions provided, create a default one
            questions_and_answers = [
                QuestionAnswer(1, "General interview evaluation", "Evaluate the candidate's overall performance based on the interview transcript")
            ]
    ...
    def from_legacy_format(cls, legacy_data: Dict[str, Any]) -> 'Interview':
        ...
        job = Job(
            title="Position",
            description=legacy_data.get("jd", "")
        )
        ...
        evaluation_materials = EvaluationMaterials(
            evaluator_prompt=legacy_data.get("system_prompt", ""),
            rubric=rubric
        )
        ...
        if full_text:
            ...
            for line in lines:
                if line.strip():
                    if line.startswith("**Interviewer:**") or line.lower().startswith("interviewer:"):
                        text = line.replace("**Interviewer:**", "").replace("interviewer:", "").strip(" '\"")
                        structured_entries.append(TranscriptEntry("interviewer", text))
                    elif line.startswith("**Candidate:**") or line.lower().startswith("candidate:"):
                        text = line.replace("**Candidate:**", "").replace("candidate:", "").strip(" '\"")
        ...
        questions_and_answers = [
            QuestionAnswer(1, "General interview questions", "Evaluate based on transcript content")
        ]
    ...
    def get_evaluation_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "completed_evaluations": sum([
                1 for eval in [self.evaluation_1, self.evaluation_2, self.evaluation_3]
                if eval is not None
            ]),
            "evaluations": {
...
if __name__ == '__main__':
    ...
    evaluation_materials = EvaluationMaterials(
        evaluator_prompt="Evaluate this candidate's technical skills",
        rubric=rubric
    )
    ...
    questions = [
        QuestionAnswer(1, "Tell me about your Python experience", "Should demonstrate depth of Python knowledge")
    ]
