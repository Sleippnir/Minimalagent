...
class QueueService:
    ...
    async def get_interview_from_queue(self, auth_token: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_evaluator_from_queue(self, interview_id: str) -> Optional[Dict[str, Any]]:
...
class EvaluationService:
    ...
    async def write_evaluation(self, interview_id: str, evaluator_llm_model: str, score: float, reasoning: str, raw_llm_response: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(f"✅ Successfully inserted {evaluator_llm_model} evaluation for interview {interview_id}")
            ...
            if len(unique_models) >= 3:
                print(f"🎯 All evaluations complete for interview {interview_id}, updating status to 'evaluated'")
                ...
                await self.client.patch("interviews", {"interview_id": interview_id}, status_update)
                print(f"✅ Successfully updated interview {interview_id} status to 'evaluated'")
            else:
                print(f"📊 {len(unique_models)}/3 evaluations completed for interview {interview_id}")
