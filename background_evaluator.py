...
from interview.evaluator.interview import Interview, Candidate, Job, Rubric, EvaluationMaterials, TranscriptData, TranscriptEntry, QuestionAnswer
...
class BackgroundEvaluatorAgent:
    ...
    async def _process_pending_evaluations(self):
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully processed evaluation: {evaluation_task.get('id')}")
            else:
                self.logger.error(f"Failed to process evaluation: {evaluation_task.get('id')}")
    ...
    async def _process_evaluation_task(self, evaluation_task: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            success = await self._store_evaluation_results(interview_id, evaluation_results)
    ...
    def _create_interview_from_payload(self, payload: Dict[str, Any]) -> Interview:
        ...
        for entry in structured_transcript_data:
            if isinstance(entry, dict):
                structured_transcript.append(TranscriptEntry(
        ...
        evaluator_prompt = eval_materials_payload.get('evaluator_prompt',
    ...
    async def _store_evaluation_results(self, interview_id: str, evaluation_results: Dict[str, Any]) -> bool:
        """
        ...
        try:
            ...
            self.context_service.interview_repo.supabase.table("interviews").update(update_data).eq("interview_id", interview_id).execute()
            
            self.logger.info(f"Successfully stored evaluation results for interview: {interview_id}")
            return True
